...
- Переписать компоненты на `React`. Имейте ввиду, что разные подходы смешивать нельзя и теперь у нас всё должно быть только по `react-way`, т.е. нельзя напрямую работать с `DOM`, как это делалось на предыдущих заданиях
...
└── src - Папка с исходниками
    ...
    │   ├── Button - Атомарный компонент
    │   │   ├── Button.jsx
    │   │   ├── Button.scss
    │   │   └── index.js
    │   └── Message - Атомарный компонент
    │       ├── Message.jsx
    │       ├── Message.scss
    │       └── index.js
